# Inference Discrepancies: LeRobot vs PhysicalAI (superseded)

> **Superseded.** This gap analysis motivated the pre-`ActionSource` design
> in `policy_runtime_design.md`. The current design (which closes these gaps
> via `RobotRuntime`/`ActionSource`) is [runtime_design.md](./runtime_design.md).
...
<details>
<summary>Original content, kept for history</summary>
