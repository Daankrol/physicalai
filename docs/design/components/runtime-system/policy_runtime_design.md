# Policy Runtime Design (superseded)

> **Superseded.** This document described the pre-`ActionSource` design
> (`PolicyRuntime` as the entry point, flat `model=`/`execution=` constructor
> args). The current design is [runtime_design.md](./runtime_design.md).
